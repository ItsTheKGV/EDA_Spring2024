...
```{r project.setup, echo = TRUE, message = FALSE, error = FALSE, results = 'hide'}
#Import libraries
...
```{r, echo = FALSE, warning = FALSE, fig.cap = "Ozone $\\mu g/m^3$ per Month in 2018", fig.align='left', fig.height=3.6}

ozone_2018Plot <- epa_data %>%
  filter(Year %in% 2018) %>%
  ggplot(aes(
    x = as.factor(Month), y = Ozone)) +
  geom_boxplot() +
  xlab("Month") +
  ylab("Ozone µg/m3")
...
```{r, echo = FALSE, warning = FALSE, fig.cap = "Ozone $\\mu g/m^3$ per Month in 2019", fig.align='left', fig.height=3.6}

ozone_2019Plot <- epa_data %>%
  filter(Year %in% 2019) %>%
  ggplot(aes(
    x = as.factor(Month), y = Ozone)) +
  geom_boxplot() +
...
https://github.com/ItsTheKGV/EDA_Spring2024
